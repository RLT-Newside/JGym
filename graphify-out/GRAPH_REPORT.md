...
## Corpus Check
- 265 files · ~99,999 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 357 nodes · 324 edges · 23 communities detected
...
- **Thin community `Community 3`** (19 nodes): `MediaBrowserBridge.java`, `MediaBrowserBridge.java`, `MediaBrowserBridge`, `.addToQueue()`, `.browse()`, `.connect()`, `.disconnect()`, `.discoverBrowserService()`, `.getFullMetadata()`, `.getQueue()`, `.isAvailable()`, `.isConnected()`, `.MediaBrowserBridge()`, `.moveQueueItem()`, `.playFromMediaId()`, `.removeQueueItem()`, `.seekTo()`, `.setRepeatMode()`, `.setShuffleMode()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 11`** (9 nodes): `MediaNotificationListener.java`, `MediaNotificationListener.java`, `MediaNotificationListener`, `.findAndAttach()`, `.onListenerConnected()`, `.onListenerDisconnected()`, `.onNotificationPosted()`, `.onNotificationRemoved()`, `NotificationListenerService`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 19`** (5 nodes): `Dashboard()`, `usePR.ts`, `dashboard.container.tsx`, `usePR()`, `useStreak()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 20`** (5 nodes): `MainActivity.java`, `MainActivity.java`, `BridgeActivity`, `MainActivity`, `.onCreate()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
